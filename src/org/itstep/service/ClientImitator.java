...
public class ClientImitator {
	...
	Good good = new Good();
	AccountDAO actionDao = new AccountDAO();
	GoodAction action = new GoodAction();
	GoodActionDAO goodaction = new GoodActionDAO();
	...
	public void addItemToWl(WebDriver driver, String login, String itemAsin) {
		...
		good = new Good(itemAsin, nameProdukt.getText(), ChToInt(price.getText()), driver.getCurrentUrl());
		// if (goodDao.get(itemAsin).getName() == null) {
		goodDao.save(good);
		// }
		wlBtnWL.click();

		action = new GoodAction(Time.valueOf(LocalTime.now()), login, itemAsin, true, false);
		goodaction.save(action);
	...
	public void addItemToCart(WebDriver driver, String login, String itemAsin) {
		...
		driver.get(goodDao.get(itemAsin).getShopUrl());

		WebElement wlBtnAC = driver.findElement(By.id("add-to-cart-button"));
		wlBtnAC.click();
		action = new GoodAction(Time.valueOf(LocalTime.now()), login, itemAsin, false, true);
		goodaction.update(itemAsin, action);
	}
